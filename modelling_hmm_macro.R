#=======================================#
#                                       #
#### HMM FOR MACROALBUMINURIA COHORT ####
#                                       #
#=======================================#

### Import Data ###
setwd(core)
DT <- read_fst("analysis_data_markov_alb3.fst", as.data.table = T)
...
set.seed(101)
uniqid <- unique(DT$patid)
ndevDT <- ceiling(length(uniqid)*0.7)
...
setwd(core)
save("testDT", file = "macro.testdata.RData")
...
macro.model <- msm(y ~ time,
                   ...
                   covariates = ~ AGE60 + SEX,
                   # fixedpars = T,
                   initprobs = initprobs)

### Matrices ###
pmatrix.msm(macro.model)
sojourn.msm(macro.model)
hazard.msm(macro.model)

### Export Model Object ###
setwd(core)
save("macro.model", file = "macro.modelv2.RData")